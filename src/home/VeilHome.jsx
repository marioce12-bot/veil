  ...
  return <div className="home-app" id="home">
    <header className="home-header">
      <div className="home-header-inner">
        ...
        <div className="home-actions"><button aria-label="Rechercher" className="mobile-search"><Search size={19} /></button><button aria-label="Messages" className="icon-button" onClick={() => navigate('messages')}><MessageCircle size={19} /><i /></button><button aria-label="Notifications" className="icon-button" onClick={() => navigate('activity')}><Bell size={19} /><i /></button><div className="profile-wrap"><button className="profile-button" onClick={() => setProfileOpen(!profileOpen)}><span className="profile-avatar">{displayName.slice(0, 1).toUpperCase()}</span><span className="profile-name">@{displayName}</span><ChevronRight size={15} /></button>{profileOpen && <div className="profile-menu"><span>Ton espace</span><button onClick={() => navigate('profile')}>Voir mon profil</button><button onClick={onSignOut}>Se déconnecter</button></div>}</div><button className="home-menu-button" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} aria-label="Ouvrir la navigation">{mobileMenuOpen ? <X size={20} /> : <Menu size={20} />}</button></div>
      </div>
    </header>
