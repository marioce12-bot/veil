  ...
  return <div className="home-app" id="home">
    ...
    <div className="home-layout">
      <main className="home-main">
        ...
        <button className="home-compose-prompt glass-panel" onClick={() => navigate('compose')}><span className="composer-avatar">◌</span><span>Qu’est-ce que tu veux dire aujourd’hui ?</span><Plus size={18} /></button>
...
function PublishView({ composerText, setComposerText, mediaFile, mediaPreview, selectMedia, setMediaFile, setMediaPreview, selectedCategory, setSelectedCategory, publish }) { const placeholders = { confession: 'Dis ce que tu n’oses pas dire.', pensee: 'Qu’est-ce que tu penses vraiment ?', debat: 'Lance une question ou donne ton opinion.', humour: 'Fais-nous sourire.' }; const labels = { confession: 'Ce que tu gardes pour toi.', pensee: 'Ce que tu veux partager.', debat: 'Lance une discussion.', humour: 'Fais sourire les autres.' }; return <section className="publish-view product-view"><div className="home-eyebrow">Nouvelle publication</div><h1>Dis-le comme tu le penses.</h1><p className="view-lead">Choisis un format, écris ce que tu veux partager et publie sous ton pseudonyme VEIL.</p><form className="publish-editor glass-panel" onSubmit={publish}><div className="publish-section-label">Type de publication</div><div className="publish-types">{categories.map((category) => <button type="button" key={category.id} className={selectedCategory === category.id ? 'selected' : ''} onClick={() => setSelectedCategory(category.id)}><span className="type-mark">{category.id === 'confession' ? 'C' : category.id === 'pensee' ? 'P' : category.id === 'debat' ? 'D' : 'H'}</span><strong>{category.label}</strong><small>{labels[category.id]}</small></button>)}</div><div className="publish-section-label">{selectedCategory === 'confession' ? 'Ta confession' : selectedCategory === 'debat' ? 'Ton débat' : selectedCategory === 'humour' ? 'Ton humour' : 'Ta pensée'}</div><div className="publish-text-wrap"><textarea maxLength={500} value={composerText} onChange={(event) => setComposerText(event.target.value)} placeholder={placeholders[selectedCategory]} rows="7" /><span>{composerText.length}/500</span></div>{mediaPreview && <div className="media-preview"><button type="button" onClick={() => { setMediaFile(null); setMediaPreview('') }} aria-label="Retirer le média"><X size={15} /></button>{mediaFile?.type.startsWith('video/') ? <video src={mediaPreview} controls /> : <img src={mediaPreview} alt="Aperçu" />}</div>}<div className="publish-section-label">Ajouter à ta publication</div><div className="publish-media-tools"><label className="media-button"><ImagePlus size={16} /> Photo<input type="file" accept="image/*" onChange={selectMedia} /></label><label className="media-button"><Video size={16} /> Vidéo<input type="file" accept="video/*" onChange={selectMedia} /></label></div><div className="publish-options"><div><span>Publication</span><small>Publier sous @{sessionStorage.getItem('veil_display_name') || 'ton pseudonyme'}</small></div><div><span>Commentaires</span><strong>Activés</strong></div><div><span>Réactions</span><strong>Activées</strong></div></div><div className="publish-editor-footer"><button className="publish-button" disabled={!composerText.trim()}>Publier anonymement</button></div></form></section> }
